...
class ResNet(nn.Sequential, Net):
    def __init__(self, input_shape, block, layers, num_filters, activation='relu', fc=True, pooling=True, num_classes=1000,
        ...
        self.append(self._make_layer(block, self.output_shape, 2 * num_filters, layers[1], stride=2, name=name + '/block2'))
        self.append(self._make_layer(block, self.output_shape, 4 * num_filters, layers[2], stride=2, name=name + '/block3'))
        self.append(self._make_layer(block, self.output_shape, 8 * num_filters, layers[3], stride=2, name=name + '/block4'))
...
class VGG16(nn.Sequential, Net):
    def __init__(self, input_shape, fc=True, bn=False, dropout=True, num_classes=1000, name='vgg16'):
...
class VGG19(nn.Sequential, Net):
    def __init__(self, input_shape, fc=True, bn=False, dropout=True, num_classes=1000, name='vgg19'):
        super(VGG19, self).__init__(input_shape=input_shape, layer_name=name)
        self.fc = fc
        self.append(nn.Conv2DLayer(self.output_shape, 64, 3, activation=None, no_bias=False, layer_name=name + '/conv1_1'))
        self.append(nn.BatchNormLayer(self.output_shape, name+'/bn1') if bn
                    else nn.ActivationLayer(self.output_shape, layer_name=name+'/relu1'))
        self.append(nn.Conv2DLayer(self.output_shape, 64, 3, activation=None, no_bias=False, layer_name=name + '/conv1_2'))
        self.append(nn.BatchNormLayer(self.output_shape, name+'/bn2') if bn
                    else nn.ActivationLayer(self.output_shape, layer_name=name+'/relu2'))
        self.append(nn.MaxPoolingLayer(self.output_shape, (2, 2), layer_name=name + '_maxpool0'))

        self.append(nn.Conv2DLayer(self.output_shape, 128, 3, activation=None, no_bias=False, layer_name=name + '/conv2_1'))
        self.append(nn.BatchNormLayer(self.output_shape, name+'/bn3') if bn
                    else nn.ActivationLayer(self.output_shape, layer_name=name+'/relu3'))
        self.append(nn.Conv2DLayer(self.output_shape, 128, 3, activation=None, no_bias=False, layer_name=name + '/conv2_2'))
        self.append(nn.BatchNormLayer(self.output_shape, name+'/bn4') if bn
                    else nn.ActivationLayer(self.output_shape, layer_name=name+'/relu4'))
        self.append(nn.MaxPoolingLayer(self.output_shape, (2, 2), layer_name=name + '_maxpool1'))

        self.append(nn.Conv2DLayer(self.output_shape, 256, 3, activation=None, no_bias=False, layer_name=name + '/conv3_1'))
        self.append(nn.BatchNormLayer(self.output_shape, name+'/bn5') if bn
                    else nn.ActivationLayer(self.output_shape, layer_name=name+'/relu5'))
        self.append(nn.Conv2DLayer(self.output_shape, 256, 3, activation=None, no_bias=False, layer_name=name + '/conv3_2'))
        self.append(nn.BatchNormLayer(self.output_shape, name+'/bn6') if bn
                    else nn.ActivationLayer(self.output_shape, layer_name=name+'/relu6'))
        self.append(nn.Conv2DLayer(self.output_shape, 256, 3, activation=None, no_bias=False, layer_name=name + '/conv3_3'))
        self.append(nn.BatchNormLayer(self.output_shape, name+'/bn7') if bn
                    else nn.ActivationLayer(self.output_shape, layer_name=name+'/relu7'))
        self.append(nn.Conv2DLayer(self.output_shape, 256, 3, activation=None, no_bias=False, layer_name=name + '/conv3_4'))
        self.append(nn.BatchNormLayer(self.output_shape, name+'/bn8') if bn
                    else nn.ActivationLayer(self.output_shape, layer_name=name+'/relu8'))
        self.append(nn.MaxPoolingLayer(self.output_shape, (2, 2), layer_name=name + '_maxpool2'))

        self.append(nn.Conv2DLayer(self.output_shape, 512, 3, activation=None, no_bias=False, layer_name=name + '/conv4_1'))
        self.append(nn.BatchNormLayer(self.output_shape, name+'/bn9') if bn
                    else nn.ActivationLayer(self.output_shape, layer_name=name+'/relu9'))
        self.append(nn.Conv2DLayer(self.output_shape, 512, 3, activation=None, no_bias=False, layer_name=name + '/conv4_2'))
        self.append(nn.BatchNormLayer(self.output_shape, name+'/bn10') if bn
                    else nn.ActivationLayer(self.output_shape, layer_name=name+'/relu10'))
        self.append(nn.Conv2DLayer(self.output_shape, 512, 3, activation=None, no_bias=False, layer_name=name + '/conv4_3'))
        self.append(nn.BatchNormLayer(self.output_shape, name+'/bn11') if bn
                    else nn.ActivationLayer(self.output_shape, layer_name=name+'/relu11'))
        self.append(nn.Conv2DLayer(self.output_shape, 512, 3, activation=None, no_bias=False, layer_name=name + '/conv4_4'))
        self.append(nn.BatchNormLayer(self.output_shape, name+'/bn12') if bn
                    else nn.ActivationLayer(self.output_shape, layer_name=name+'/relu12'))
        self.append(nn.MaxPoolingLayer(self.output_shape, (2, 2), layer_name=name + '_maxpool3'))

        self.append(nn.Conv2DLayer(self.output_shape, 512, 3, activation=None, no_bias=False, layer_name=name + '/conv5_1'))
        self.append(nn.BatchNormLayer(self.output_shape, name+'/bn13') if bn
                    else nn.ActivationLayer(self.output_shape, layer_name=name+'/relu13'))
        self.append(nn.Conv2DLayer(self.output_shape, 512, 3, activation=None, no_bias=False, layer_name=name + '/conv5_2'))
        ...
        self.append(nn.Conv2DLayer(self.output_shape, 512, 3, activation=None, no_bias=False, layer_name=name + '/conv5_3'))
        ...
        self.append(nn.Conv2DLayer(self.output_shape, 512, 3, activation=None, no_bias=False, layer_name=name + '/conv5_4'))
